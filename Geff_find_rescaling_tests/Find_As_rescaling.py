...
A_s =  1.53e-9 ## Original amplitude 

## Both simulations have the same h. 

massless = np.loadtxt("Geff_find_rescaling_tests/lcdm_base_00_pk.dat", skiprows=4)
k_massless = massless[:,0]
Pk_massless = massless[:,1]

massive = np.loadtxt("Geff_find_rescaling_tests/lcdm_nu_00_pk.dat", skiprows=4)
k_massive = massive[:,0]
...
rescaled = np.loadtxt("Geff_find_rescaling_tests/lcdm_rescaled_00_pk.dat", skiprows=4)
